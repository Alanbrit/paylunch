...
@section('content')
    ...
    <form action="{{ route('mensaje.update', $mensaje->id)}}" method="post" class="w-full">
        @csrf
        @method("PUT")
        <div class="flex flex-wrap mb-2">
            <div class="w-full md:w-1/3 px-3 mb-6 md:mb-0">
                <label for="" class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2">Contenido</label>
                <textarea name="contenido" class="form-textarea mt-1 block w-full" rows="3">{{ $mensaje->contenido }}</textarea>
            </div>
            <div class="w-full md:w-1/3 px-3 mb-6 md:mb-0">
                ...
                <select name="id_receptor" class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                    <option></option>
                    @foreach ($users as $u)
                        <option {{ $mensaje->id_receptor == $u->id ? 'selected' : '' }} value="{{ $u->id }}"> {{ $u->name }} {{ $u->apellidos }} </option>
                    @endforeach
                </select>
            </div>
            <div class="w-full md:w-1/3 px-3 mb-6 md:mb-0">
